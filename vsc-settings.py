...
@click.option('--settings-dir', default=path.expanduser('~/Library/Application Support/Code/User'))
...
def export_settings(settings_dir, out_dir):
    ...
    for f in ['settings.json', 'keybindings.json', 'projects.json', 'snippets']:
        ...
        if path.exists(ff):
            ...
            subprocess.run('cp -r {} {}'.format('~/Library/Application\ Support/Code/User', dump_path), shell=True)

    now = dt.datetime.now()
    ball_path = path.join(out_dir, 'export-{}.zip'.format(now.strftime("%Y-%m-%d-%H%M%S")))
    subprocess.run(
